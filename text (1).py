...
class bus_booking:
    ...
    def cover_page(self):
        ...
        Label(root, text="\n\n\n\n\nName: Gaurangi Tripathi", font='Arial 16 bold', fg='blue').pack()
        Label(root, text="\nEnr : 221b161", font='Arial 16 bold', fg='blue').pack()
...
con = sqlite3.connect('new database')
cur=con.cursor()
cur.execute('create table if not exists bus(bus_id varchar(5)not null primary key,bus type varchar,bus capacity int,fair int,op_id varchar(5) not null,route_id varchar(5) not null,foreign key(op_id) references operator(opr_id),foreign key (route_id) referneces route (r id)')
cur.execute('create table if not exists operator(opr_id varchar(5) primary key,name varchar(20),address varchar(40),phone char(10),email_id varchar(30))')
cur.execute('create table if not exists running(b_id varchar(5),run_date date,seat_avl int,foreign key (b_id),references bus(bus_id))')
cur.execute('create table if not exists route(r_id varchar(5) not null primary key,s_name varchar(20),s_id varchar(5),e_name varchar(20),e_id varchar(5))')
cur.execute('create table if not exists booking_history(name varchar(20),gender varchar(1),no_of_seats int,phone char(10),age int,booking_ref varchar(10) not primary key,booking_date date,travel_date date,bid varchar(5),foreign key(bid) references bus(bus_id))')
...
class bus_booking:
    ...
    def home_page(self):
        ...
        Label(root,text='\n\n\n\n\n').grid(row=0,column=0)
        Label(root,image=bus).grid(row=1,column=1,columnspan=12 ,padx= w //2.5)
        Label(root,text="Online Bus Booking System",font='Arial 20 bold',bg='light blue').grid(row=2,column=2,columnspan=9 ,padx= w //2.5)
    ...
    def cover_page(self):
            ...
            root = Tk()
            root.title("COVER PAGE")
            #img=Image.open('D:\project\bgimage.jpg')
            #img=PhotoImage(file="D:\project\bgimage.jpg")
           # bgimg=label(root,image=img)
           # bgimg.place(relheight=1,relwidth=1)
           # bgimage=Image.open('bgimage.JPG')
            #re=bgimahrtf5t6ge.resize((300,300))
            #nebgimage=ImageTk.PhotoImage(bgimage)

            
            bus=Image.open('Bus_for_project.png')
            rebus=bus.resize((300,300))
            nebus=ImageTk.PhotoImage(rebus)
            h,w =root.winfo_screenheight(),root.winfo_screenwidth()
            root.geometry('%dx%d+0+0'%(w,h))
            #relb= Label(root, image=nebgimage, width=bgimage.width, height=bgimage.height)h
           # relb.place(x=0,y=0)
            Label(root,text='\n\n\n\n\n').pack()
            Label(root, image=nebus).pack()
            Label(root,text="Online Bus Booking System",font='Arial 20 bold',bg='light blue').pack()
            Label(root, text="\n\n\n\n\nName: Anurag Sharma", font='Arial 16 bold', fg='blue').pack()
            Label(root, text="\nEnr : 221b081", font='Arial 16 bold', fg='blue').pack()
            Label(root, text="\nMobile : 9798318799\n\n\n\n", font='Arial 16 bold', fg='blue').pack()
            Label(root, text="Submitted to: Dr. Mahesh Kumar", font='Arial 15 bold', bg='light blue', fg='red').pack()
            Label(root, text="Project Based Learning", font='Arial 15 bold', fg='red').pack()
            def cover_to_home(event):
